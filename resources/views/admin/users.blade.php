                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-[#1a1a18] transition-all">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        ...
                                        @if($u->id !== Auth::id())
                                            <form action="{{ route('admin.web.users.destroy', $u->id) }}" method="POST" class="inline" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer définitivement l\'utilisateur {{ addslashes($u->full_name) }} ? Cette action est irréversible.');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex items-center gap-1 text-xs font-semibold text-red-600 hover:text-red-800">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
